...
static void iperf_udp_client(void *thread_param)
{
    ...
    server.sin_port = lwip_htons(param.port);
    ...
    while (param.mode != IPERF_MODE_STOP)
    {
        ...
        ret = lwip_sendto(sock, buffer, send_size, 0, (struct sockaddr *)&server, sizeof(struct sockaddr_in));
        if (ret > 0)
        {
            sentlen += ret;
        }
...
static void iperf_client(void *thread_param)
{
    ...
    while (param.mode != IPERF_MODE_STOP)
    {
        ...
        addr.sin_port = lwip_htons(param.port);
        ...
        while (param.mode != IPERF_MODE_STOP)
        {
            ...
            ret = lwip_send(sock, send_buf, IPERF_BUFSZ, 0);
            if (ret > 0)
            {
                sentlen += ret;
            }
...
void iperf_server(void *thread_param)
{
    ...
    while (param.mode != IPERF_MODE_STOP)
    {
        ...
        if (lwip_select(sock + 1, &readset, RT_NULL, RT_NULL, &timeout) == 0)
            continue;
        ...
        while (param.mode != IPERF_MODE_STOP)
        {
            bytes_received = lwip_recv(connected, recv_data, IPERF_BUFSZ, 0);
