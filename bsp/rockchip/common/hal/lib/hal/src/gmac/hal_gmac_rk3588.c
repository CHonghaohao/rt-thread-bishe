...
#if defined(SOC_RK3588) && defined(HAL_GMAC_MODULE_ENABLED)
...
#define RK3588_SYS_GRF_SOC_CON8  0X0320    //GMAC0 CON0
#define RK3588_SYS_GRF_SOC_CON9  0X0324    //GMAC1 CON0
#define RK3588_PHP_GRF_GMAC_CON0 0X0008    //GMAC0 GMAC1 MODE
#define RK3588_SYS_GRF_SOC_CON7  0X031C    //GMAC0 GMAC1 RX/TX CLK_DLY

 //   #define RK3568_GMAC_GMII_MODE GRF_BIT(7)
 /* RK3588_PHP_GRF_GMAC_CON0 && RK3588_SYS_GRF_SOC_CON71 */
//RGMII:001 RMII:100
//RGMII:001 RMII:100
 #define RK3588_GMAC0_PHY_INTF_SEL_RGMII \
         (GRF_BIT(3) | GRF_CLR_BIT(4) | GRF_CLR_BIT(5))
 #define RK3568_GMAC0_PHY_INTF_SEL_RMII \
         (GRF_CLR_BIT(3) | GRF_CLR_BIT(4) | GRF_BIT(5))
 #define RK3588_GMAC1_PHY_INTF_SEL_RGMII \
         (GRF_BIT(9) | GRF_CLR_BIT(10) | GRF_CLR_BIT(11))
 #define RK3568_GMAC1_PHY_INTF_SEL_RMII \
         (GRF_CLR_BIT(9) | GRF_CLR_BIT(10) | GRF_BIT(11))

 #define RK3588_GMAC0_FLOW_CTRL         GRF_BIT(0)
 #define RK3588_GMAC0_FLOW_CTRL_CLR     GRF_CLR_BIT(0)
 #define RK3588_GMAC1_FLOW_CTRL         GRF_BIT(6)
 #define RK3588_GMAC1_FLOW_CTRL_CLR     GRF_CLR_BIT(6)

 #define RK3588_GMAC0_RXCLK_DLY_ENABLE  GRF_BIT(3)
 #define RK3588_GMAC0_RXCLK_DLY_DISABLE GRF_CLR_BIT(3)
 #define RK3588_GMAC0_TXCLK_DLY_ENABLE  GRF_BIT(2)
 #define RK3588_GMAC0_TXCLK_DLY_DISABLE GRF_CLR_BIT(2)
 #define RK3588_GMAC1_RXCLK_DLY_ENABLE  GRF_BIT(5)
 #define RK3588_GMAC1_RXCLK_DLY_DISABLE GRF_CLR_BIT(5)
 #define RK3588_GMAC1_TXCLK_DLY_ENABLE  GRF_BIT(4)
 #define RK3588_GMAC1_TXCLK_DLY_DISABLE GRF_CLR_BIT(4)
 
 /* RK3588_SYS_GRF_SOC_CON8 && RK3588_SYS_GRF_SOC_CON9 */
// 0X7F00(val)00
                        ...
                        int32_t txDelay, int32_t rxDelay)
{
    uint32_t *con0, *con1;
    if (pGMAC->pReg == GMAC0) {
        WRITE_REG((PHP_GRF->GMAC_CON0), RK3588_GMAC0_PHY_INTF_SEL_RGMII);
        WRITE_REG((SYS_GRF->SOC_CON7), RK3588_GMAC0_TXCLK_DLY_ENABLE);
        WRITE_REG((SYS_GRF->SOC_CON8), RK3588_GMAC_CLK_RX_DL_CFG(0) | RK3588_GMAC_CLK_TX_DL_CFG(txDelay));
    }
    else {
        if (pGMAC->pReg == GMAC1) {
            WRITE_REG((PHP_GRF->GMAC_CON0), RK3588_GMAC1_PHY_INTF_SEL_RGMII);
            WRITE_REG((SYS_GRF->SOC_CON7), RK3588_GMAC1_TXCLK_DLY_ENABLE);
            WRITE_REG((SYS_GRF->SOC_CON9), RK3588_GMAC_CLK_RX_DL_CFG(0) | RK3588_GMAC_CLK_TX_DL_CFG(txDelay));
        }
        else {}
    }
}
...
void HAL_GMAC_SetExtclkSrc(struct GMAC_HANDLE *pGMAC, bool extClk)
{
    ...
    // }
    if (pGMAC->pReg == GMAC0) {
            val = HIWORD_UPDATE(clksel, 0x1, 4);
            WRITE_REG((PHP_GRF->CLK_CON1), val);
    }
    else {
        if (pGMAC->pReg == GMAC1) {
            val = HIWORD_UPDATE(clksel, 0x1, 9);
            WRITE_REG((PHP_GRF->CLK_CON1), val);
        }
        else {}
    }
...
#endif /* SOC_RK3588 && HAL_GMAC_MODULE_ENABLED */
